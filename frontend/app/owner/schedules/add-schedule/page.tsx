...
const AddSchedule = () => {
  ...
  const onSubmit: SubmitHandler<AddValidationSchema> = (data) => {
    authorizedRequest
      .post(`api/schedule/${regionId}/${type}`, { ...data, regionName })
      ...
      <PageTransition>
        ...
        <div className="inset-0 z-50 overflow-auto flex justify-center items-center">
          <div className="bg-slate-200 rounded-sm px-16 max-w-lg mx-auto shadow-lg">
            {type === "i" && (
              <h1 className="pt-10 text-2xl">Add Irrigation Schedule</h1>
            )}
            {type === "f" && (
              <h1 className="pt-10 text-2xl">Add Fertilizer Schedule</h1>
            )}
            <form
